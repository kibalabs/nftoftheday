...
def create_api(galleryManager: GalleryManager, responseBuilder: ResponseBuilder) -> APIRouter:
    ...
    @router.get('/collections/{registryAddress}/tokens', response_model=GetCollectionTokenRecentSalesResponse)
    async def get_collection_tokens(registryAddress: str, attributeName: Optional[str], attributeValue: Optional[str],attributeName2: Optional[str], attributeValue2: Optional[str], limit: Optional[int] = None, offset: Optional[int] = None):
