...
class SubCollectionManager:
    ...
    async def update_sub_collection_deferred(self, collectionName: str, shouldForce: Optional[bool] = False) -> None:
        if not shouldForce:
            recentlyUpdatedCollections = await self.retriever.list_collections(
                fieldFilters=[
                    StringFieldFilter(fieldName=SubCollectionsTable.c.collectionName.key, eq=collectionName),
    ...
    async def update_sub_collection(self, collectionName: str,shouldForce: Optional[bool] = False) -> None:
        externalId = '0'
        if not shouldForce:
            recentlyUpdatedSubCollections = await self.retriever.list_sub_collections(
                fieldFilters=[
                    StringFieldFilter(fieldName=SubCollectionsTable.c.name.key, eq=collectionName),
                    DateFieldFilter(fieldName=SubCollectionsTable.c.updatedDate.key, gt=date_util.datetime_from_now(days=-_SUB_COLLECTION_UPDATE_MIN_DAYS))
                ],
            )
            if len(recentlyUpdatedSubCollections) > 0:
                ...
                return
        try:
            retrievedSubCollection = await self.subCollectionProcessor.retrieve_sub_collection(collectionName=collectionName)
        except SubCollectionDoesNotExist:
            logging.info(f'Failed to retrieve non-existent collection: {collectionName}')
            return
        async with self.saver.create_transaction() as connection:
            try:
                subCollection = await self.retriever.get_sub_collection_by_registry_address_external_id(connection=connection, collectionName=collectionName, externalId=externalId)
            ...
            if subCollection:
                ...
                await self.saver.create_sub_collection(connection=connection, collectionName=collectionName, name=retrievedSubCollection.name, symbol=retrievedSubCollection.symbol, description=retrievedSubCollection.description, imageUrl=retrievedSubCollection.imageUrl, twitterUsername=retrievedSubCollection.twitterUsername, instagramUsername=retrievedSubCollection.instagramUsername, wikiUrl=retrievedSubCollection.wikiUrl, openseaSlug=retrievedSubCollection.openseaSlug, url=retrievedSubCollection.url, discordUrl=retrievedSubCollection.discordUrl, bannerImageUrl=retrievedSubCollection.bannerImageUrl, doesSupportErc721=retrievedSubCollection.doesSupportErc721, doesSupportErc1155=retrievedSubCollection.doesSupportErc1155)
