...
export class NotdClient extends ServiceClient {
  ...
  public getTokenRecentSales = async (registryAddress: string, tokenId: string, offset?: number, limit?: number): Promise<Resources.TokenTransfer[]> => {
    const method = RestMethod.GET;
    const path = `v1/collections/${registryAddress}/tokens/${tokenId}/recent-sales`;
    const request = new Endpoints.GetTokenRecentSalesRequest(offset, limit);
