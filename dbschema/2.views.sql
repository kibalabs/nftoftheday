...
CREATE MATERIALIZED VIEW mvw_user_registry_ordered_ownerships AS (
    SELECT registry_address, token_id, owner_address, sum(quantity) as quantity , max(owner_token_index) as owner_token_index from
    ((
    ...
    )) as user_registry_ordered_ownerships
    GROUP BY registry_address, token_id, owner_address
);
